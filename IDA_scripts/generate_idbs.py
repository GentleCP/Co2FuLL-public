...
LOG_PATH = CUR_PATH.joinpath("generate_idbs.log")
...
def main():
    ...
    for proj_path in ROOT_PATH.joinpath("DBs/Binkit-1.0-normal-strip-binaries").iterdir():
        for bin_path in proj_path.iterdir():
            idb_path = str(bin_path).replace('DBs', 'IDBs') + '.'
        bin_info['idb_path'].parent.mkdir(exist_ok=True, parents=True)
        params.append(bin_info)
    results = execute_by_multi_process(export_idb, params, n_jobs=N_JOBS)
    df_res = pd.DataFrame(results)
    if df_res is not None:
        logger.critical(df_res['errcode'].value_counts())
        df_res.to_csv('generate_idbs_res.csv', index=False)
